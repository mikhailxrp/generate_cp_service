...
async function importCpo90() {
  try {
    console.log("🔄 Начинаю импорт элементов CPO90 (PRICE_CPO90)...");
    ...
    if (!workbook.SheetNames.includes("PRICE_CPO90")) {
      throw new Error("Лист PRICE_CPO90 не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_CPO90"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "cpo90"))
      .limit(1);

    if (!category) throw new Error("Категория cpo90 не найдена!");

    console.log(`✅ Категория cpo90 найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapCpo90RowToPriceItem(row, category.id);

        await db.insert(priceItems).values(mapped);
        console.log(
          `➕ Добавлен элемент CPO90: ${mapped.title} (${mapped.sku})`
        );
        inserted++;
      } catch (err) {
