...
const mapSmartmeterRowToPriceItem = (row, categoryId) => {
  const meta = {
  ...
  const attrs = {
    electrical: {
      phases: toInt(row["Выход_фазы(1/3)"]), // 1 или 3
    },
    compat: {
      inverter_type: row["Тип_инвертора"] || null, // просто сохраняем текстом
    },
    bos: {
...
async function importSmartmeters() {
  try {
    console.log("🔄 Начинаю импорт смартметров (PRICE_SMARTMETER)...");
    ...
    if (!workbook.SheetNames.includes("PRICE_SMARTMETER")) {
      throw new Error("Лист PRICE_SMARTMETER не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_SMARTMETER"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "smartmeter"))
      .limit(1);

    if (!category) throw new Error("Категория smartmeter не найдена!");

    console.log(`✅ Категория smartmeter найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapSmartmeterRowToPriceItem(row, category.id);
        ...
        console.log(`➕ Добавлен смартметр: ${mapped.title} (${mapped.sku})`);
        inserted++;
      } catch (err) {
