...
const mapCs90RowToPriceItem = (row, categoryId) => {
  const meta = {
  ...
  const attrs = {
    mechanical: {
      ...
      dimensions_mm:
        row["Размеры_мм (В×Ш×Д)"] || row["Размеры_мм(Д×Ш×Т)"] || null,
    },
    bos: {
...
async function importCs90() {
  try {
    console.log("🔄 Начинаю импорт элементов CS90 (PRICE_CS90)...");
    ...
    if (!workbook.SheetNames.includes("PRICE_CS90")) {
      throw new Error("Лист PRICE_CS90 не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_CS90"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "cs90"))
      .limit(1);

    if (!category) throw new Error("Категория cs90 не найдена!");

    console.log(`✅ Категория cs90 найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapCs90RowToPriceItem(row, category.id);

        await db.insert(priceItems).values(mapped);
        console.log(
          `➕ Добавлен элемент CS90: ${mapped.title} (${mapped.sku})`
        );
        inserted++;
      } catch (err) {
