...
const mapLotkiRowToPriceItem = (row, categoryId) => {
  const meta = {
    ...
    tray_type: row["Тип_лотка"] || null, // перфорированный/сплошной и т.п.
...
async function importLotki() {
  try {
    console.log("🔄 Начинаю импорт лотков (PRICE_LOTKI)...");
    ...
    if (!workbook.SheetNames.includes("PRICE_LOTKI")) {
      throw new Error("Лист PRICE_LOTKI не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_LOTKI"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "lotki"))
      .limit(1);

    if (!category) throw new Error("Категория lotki не найдена!");

    console.log(`✅ Категория lotki найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapLotkiRowToPriceItem(row, category.id);

        await db.insert(priceItems).values(mapped);
        console.log(`➕ Добавлен лоток: ${mapped.title} (${mapped.sku})`);
        inserted++;
      } catch (err) {
