...
async function importCable() {
  try {
    console.log("🔄 Начинаю импорт кабеля (PRICE_CABLE)...");
    ...
    if (!workbook.SheetNames.includes("PRICE_CABLE")) {
      throw new Error("Лист PRICE_CABLE не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_CABLE"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "cable"))
      .limit(1);

    if (!category) throw new Error("Категория cable не найдена!");

    console.log(`✅ Категория cable найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapCableRowToPriceItem(row, category.id);
        ...
        console.log(`➕ Добавлен кабель: ${mapped.title} (${mapped.sku})`);
        inserted++;
      } catch (err) {
