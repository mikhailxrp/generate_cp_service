...
const mapDptTRowToPriceItem = (row, categoryId) => {
  const meta = {
...
async function importDptT() {
  try {
    console.log("🔄 Начинаю импорт DPT-T (PRICE_DPT-T)...");
    ...
    if (!workbook.SheetNames.includes("PRICE_DPT-T")) {
      throw new Error("Лист PRICE_DPT-T не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_DPT-T"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "dpt_t"))
      .limit(1);

    if (!category) throw new Error("Категория dpt_t не найдена!");

    console.log(`✅ Категория dpt_t найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapDptTRowToPriceItem(row, category.id);

        await db.insert(priceItems).values(mapped);
        console.log(
          `➕ Добавлен DPT-T элемент: ${mapped.title} (${mapped.sku})`
        );
        inserted++;
      } catch (err) {
