...
async function importPowOff() {
  try {
    console.log(
      "🔄 Начинаю импорт силовых выключателей / вводных автоматов (PRICE_POW_OFF)..."
    );
    ...
    if (!workbook.SheetNames.includes("PRICE_POW_OFF")) {
      throw new Error("Лист PRICE_POW_OFF не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_POW_OFF"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "pow_off"))
      .limit(1);

    if (!category) throw new Error("Категория pow_off не найдена!");

    console.log(`✅ Категория pow_off найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapPowOffRowToPriceItem(row, category.id);
        ...
        console.log(
          `➕ Добавлен выключатель/автомат: ${mapped.title} (${mapped.sku})`
        );
        inserted++;
      } catch (err) {
