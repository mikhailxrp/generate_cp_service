...
const mapKrepLotkiRowToPriceItem = (row, categoryId) => {
  const meta = {
  ...
  const attrs = {
    ...
    compat: {
      roof_flat: toBool(row["Тип_кровли_плоская"]),
      roof_metal: toBool(row["Тип_кровли_металл"]),
    },
    bos: {
...
async function importKrepLotki() {
  try {
    console.log("🔄 Начинаю импорт крепежа лотков (PRICE_KREP_LOTKI)...");
    ...
    if (!workbook.SheetNames.includes("PRICE_KREP_LOTKI")) {
      throw new Error("Лист PRICE_KREP_LOTKI не найден.");
    }

    const rows = XLSX.utils.sheet_to_json(workbook.Sheets["PRICE_KREP_LOTKI"]);
    console.log(`📊 Найдено строк: ${rows.length}`);

    const [category] = await db
      .select()
      .from(priceCategories)
      .where(eq(priceCategories.code, "krep_lotki"))
      .limit(1);

    if (!category) throw new Error("Категория krep_lotki не найдена!");

    console.log(`✅ Категория krep_lotki найдена. ID = ${category.id}`);
    ...
    for (const row of rows) {
      ...
      try {
        const mapped = mapKrepLotkiRowToPriceItem(row, category.id);

        await db.insert(priceItems).values(mapped);
        console.log(
          `➕ Добавлен крепёж лотков: ${mapped.title} (${mapped.sku})`
        );
        inserted++;
      } catch (err) {
    ...
    console.log("\n🎉 Импорт PRICE_KREP_LOTKI завершён.");
